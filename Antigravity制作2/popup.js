// popup.js — Main logic for Free Media Search Extension
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ===== State =====
    let apiKeys = null;
    let currentType = 'images'; // 'images' | 'videos'
    let isSearching = false;
    let lastQuery = '';
    let currentPage = 1;
    let currentOrientation = 'all'; // all | landscape | portrait | square
    let currentSource = 'all'; // all | pexels | pixabay
    ...
    async function searchPexelsImages(query, perPage, page) {
        ...
        return (data.photos || []).map(function (photo) {
            return {
                id: photo.id,
                source: 'Pexels',
                title: photo.alt || ('Pexels — ' + photo.photographer),
    ...
    async function searchPexelsVideos(query, perPage, page) {
        ...
        return (data.videos || []).map(function (video) {
            ...
            return {
                id: video.id,
                source: 'Pexels',
                title: 'Pexels — ' + userName,
                author: userName,
                thumbnail: video.image,
                preview: previewFile.link || '',
    ...
    async function searchPixabayVideos(query, perPage, page) {
        ...
        if (!res.ok) throw new Error('Pixabay API error: ' + res.status);
        const data = await res.json();
        console.log('[DEBUG-Popup] Pixabay Video Response:', data);

        return (data.hits || []).map(function (hit) {
            console.log('[DEBUG-Popup] Pixabay Hit:', hit.id, { picture_id: hit.picture_id, videos: hit.videos });
    ...
    async function performSearch(query, opts) {
        ...
        if (currentSource === 'pexels' && !hasPexelsKey()) {
            setStatus('Pexels APIキーが設定されていません', 'error');
            return;
        }
        if (currentSource === 'pixabay' && !hasPixabayKey()) {
            setStatus('Pixabay APIキーが設定されていません', 'error');
            return;
        }
        if (!doPexels && !doPixabay) {
            setStatus('検索可能なAPIキーがありません', 'error');
            return;
        }
        ...
        try {
            ...
            if (doPexels) {
                if (currentType === 'images') {
                    promises.push(searchPexelsImages(lastQuery, 20, currentPage));
                } else {
                    promises.push(searchPexelsVideos(lastQuery, 20, currentPage));
                }
            ...
            if (doPixabay) {
                if (currentType === 'images') {
                    promises.push(searchPixabayImages(lastQuery, 20, currentPage));
                } else {
                    promises.push(searchPixabayVideos(lastQuery, 20, currentPage));
                }
    ...
    function createCard(item, index) {
        ...
        if (item.type === 'video') {
            ...
            console.log('[DEBUG-Popup] Creating Video Card:', item.id, { videoSrc: videoSrc, posterUrl: posterUrl, source: item.source });
        ...
        card.innerHTML =
            ...
            thumbHtml +
            '<span class="card-source-badge ' + badgeClass + '">' + escapeHtml(item.source) + '</span>' +
            '<div class="card-preview-icon">' + previewIconSvg + '</div>' +
            '</div>' +
            '<div class="card-info">' +
            ...
            '<a class="card-btn btn-source" href="' + escapeAttr(item.sourceUrl) + '" target="_blank" rel="noopener">' +
            ...
            '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>' +
            'Download</button>' +
            '</div>' +
            '</div>';

        var thumb = card.querySelector('.card-thumb');
        thumb.addEventListener('click', function () { openPreview(item); });

        var dlBtn = card.querySelector('.btn-download');
        dlBtn.addEventListener('click', function (e) {
            e.stopPropagation();
            handleDownload(dlBtn, item);
        });
    ...
    function handleDownload(btn, item) {
        ...
        try {
            chrome.runtime.sendMessage(
                { action: 'download', url: item.downloadUrl, filename: filename },
                function (response) {
    ...
    function openPreview(item) {
        ...
        if (item.type === 'video') {
            ...
            video.src = item.preview || item.downloadUrl;
            video.controls = true;
            video.autoplay = true;
            ...
            img.src = item.preview || item.thumbnail;
            img.alt = item.title;
    ...
    if (refreshBtn) {
        refreshBtn.addEventListener('click', function () {
            if (lastQuery) performSearch(lastQuery, { keepPage: true });
        });
    }
    ...
    if (prevBtn) {
        prevBtn.addEventListener('click', function () {
            if (currentPage > 1) {
                currentPage--;
                performSearch(lastQuery, { keepPage: true });
            }
        });
    }
    ...
    // Full search button
    var fullSearchBtn = document.getElementById('fullSearchBtn');
    if (fullSearchBtn) {
    ...
    chrome.storage.onChanged.addListener(function (changes, namespace) {
        if (namespace === 'local' && (changes.pexelsKey || changes.pixabayKey)) {
            console.log('API keys changed in storage, updating state...');
            loadApiKeys().then(function (keys) {
                ...
                if (!lastQuery) {
                    ...
                    setStatus('API keys updated — ready to search', 'success');
                }
            }).catch(function (err) {
                if (err.message === 'NO_KEYS') {
                    apiKeys = null;
    ...
    loadApiKeys().then(function (keys) {
        ...
        if (err.message === 'NO_KEYS') {
            // Don't disable button here if we want to allow user to try and hit notice
            // But we'll show notice immediately
            showApiKeyNotice();
            setStatus('APIキーが未設定です — 設定画面で入力してください', 'error');
        } else {
